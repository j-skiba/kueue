...
func TestEffectiveResourceGroups(t *testing.T) {
	...
	cqEmptyEffective := utiltestingapi.MakeClusterQueue("test-cq").
		ResourceGroup(specFlavor).
		Obj()
	cqEmptyEffective.Status.EffectiveQuotas = &kueue.EffectiveQuotaStatus{ResourceGroups: []kueue.ResourceGroup{}}

	cohortEmptyEffective := utiltestingapi.MakeCohort("test-cohort").
		ResourceGroup(specFlavor).
		Obj()
	cohortEmptyEffective.Status.EffectiveQuotas = &kueue.EffectiveQuotaStatus{ResourceGroups: []kueue.ResourceGroup{}}
	...
	cases := map[string]struct {
		...
		"DynamicQuota enabled returns status effectiveQuotas when set": {
			dynamicQuota: true,
			cq:           cq,
			cohort:       cohort,
			wantRGs:      statusRGs,
		},
		"DynamicQuota enabled returns spec when effectiveQuotas is nil": {
			dynamicQuota: true,
			cq:           cqNoEffective,
			cohort:       cohortNoEffective,
			wantRGs:      specRGs,
		},
		"DynamicQuota enabled returns empty status effectiveQuotas when set to empty": {
			dynamicQuota: true,
			cq:           cqEmptyEffective,
