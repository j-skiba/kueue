...
// EffectiveResourceGroups returns Status.EffectiveQuota.ResourceGroups when DynamicQuotaOrchestration
...
func EffectiveResourceGroups(cq *kueue.ClusterQueue) []kueue.ResourceGroup {
	...
	if features.Enabled(features.DynamicQuotaOrchestration) && cq.Status.EffectiveQuota != nil {
		return cq.Status.EffectiveQuota.ResourceGroups
	}
	return cq.Spec.ResourceGroups
}

// EffectiveCohortResourceGroups returns Status.EffectiveQuota.ResourceGroups when DynamicQuotaOrchestration
// feature gate is enabled and effective quota is set; otherwise returns Spec.ResourceGroups.
func EffectiveCohortResourceGroups(cohort *kueue.Cohort) []kueue.ResourceGroup {
	if cohort == nil {
		return nil
	}
	if features.Enabled(features.DynamicQuotaOrchestration) && cohort.Status.EffectiveQuota != nil {
