...
func TestSchedule(t *testing.T) {
	...
	cases := map[string]scheduleTestCase{
		...
		"admit workload using EffectiveQuota when DynamicQuota is enabled": {
			featureGates: map[featuregate.Feature]bool{features.DynamicQuota: true},
			additionalClusterQueues: []kueue.ClusterQueue{
				*utiltestingapi.MakeClusterQueue("eff-cq").
					NamespaceSelector(&metav1.LabelSelector{}).
					ResourceGroup(*utiltestingapi.MakeFlavorQuotas("default").
						Resource(corev1.ResourceCPU, "0").Obj()).
					EffectiveResourceGroup(*utiltestingapi.MakeFlavorQuotas("default").
						Resource(corev1.ResourceCPU, "10").Obj()).
					Obj(),
			},
			additionalLocalQueues: []kueue.LocalQueue{
		...
		"ignore EffectiveQuota when DynamicQuota is disabled": {
			featureGates: map[featuregate.Feature]bool{features.DynamicQuota: false},
			additionalClusterQueues: []kueue.ClusterQueue{
			...
			wantWorkloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("eff-wl-disabled", "default").
					...
					Condition(metav1.Condition{
						...
						Status:             metav1.ConditionFalse,
						Reason:             kueue.WorkloadQuotaReservedReasonExceedsMaxQuota,
						Message:            "couldn't assign flavors to pod set ps: insufficient quota for cpu in flavor default, previously considered podsets requests (0) + current podset request (5) > maximum capacity (0)",
						LastTransitionTime: metav1.NewTime(now),
					}).
					Condition(metav1.Condition{
						Type:               kueue.WorkloadAdmitted,
