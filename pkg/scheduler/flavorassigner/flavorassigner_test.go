...
func TestReclaimBeforePriorityPreemption(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			flvAssigner := New(wlInfo, testClusterQueue, resourceFlavors, false, &testOracle{tc.simulationResult}, nil, nil, nil, configapi.QuotaCheckBlockUndeclared)
			assignment := flvAssigner.Assign(log, nil)
	...
	for name, tc := range cases {
		for _, enabled := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/gate_%t", name, enabled), func(t *testing.T) {
				...
				flvAssigner := New(wlInfo, clusterQueue, flavorMap, false, &testOracle{}, nil, nil, nil, configapi.QuotaCheckBlockUndeclared)
...
func TestHierarchical(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			flvAssigner := New(wlInfo, testClusterQueue, resourceFlavors, false, &testOracle{}, nil, nil, nil, configapi.QuotaCheckBlockUndeclared)
			assignment := flvAssigner.Assign(log, nil)
			if gotRepMode := assignment.RepresentativeMode(); gotRepMode != tc.wantMode {
				t.Errorf("Unexpected RepresentativeMode. got %s, want %s", gotRepMode, tc.wantMode)
			}
...
func TestAssignFlavorsWithAllowedFlavors(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assigner := New(wlInfo, cqSnapshot, resourceFlavors, false, &testOracle{}, nil, nil, nil, configapi.QuotaCheckBlockUndeclared)
...
func TestIsNoFitDueToCapacityAndLimits(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assigner := New(wlInfo, cqSnapshot, testFlavors, false, &testOracle{simulationResult: tc.simulationResult}, tc.replaceWl, nil, nil, configapi.QuotaCheckBlockUndeclared)
			gotAssignment := assigner.Assign(log, nil)
