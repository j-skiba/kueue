...
func (s *Scheduler) processEntry(
	...
	if mode == flavorassigner.NoFit {
		...
		if isNoFitDueToCapacity(&e.assignment) {
			e.quotaReservedReason = kueue.WorkloadQuotaReservedReasonPendingCapacity
		}
...
func (s *Scheduler) requeueAndUpdate(ctx context.Context, e entry) {
	...
	if e.status == notNominated || e.status == skipped || e.status == preemptionGated {
		wl := e.Obj.DeepCopy()
		reason := "Pending"
		message := e.inadmissibleMsg
		if features.Enabled(features.WorkloadUnadmittedObservability) {
			targetReason := e.quotaReservedReason
			if targetReason == "" {
				targetReason = kueue.WorkloadQuotaReservedReasonPendingCapacity
			}
			if currentCond := apimeta.FindStatusCondition(wl.Status.Conditions, kueue.WorkloadQuotaReserved); currentCond != nil && currentCond.Status == metav1.ConditionFalse {
				reason, message = workload.ResolveQuotaReservedReasonAndMessage(currentCond.Reason, currentCond.Message, targetReason, e.inadmissibleMsg)
			} else {
				reason, message = targetReason, e.inadmissibleMsg
			}
		}
		if err := workload.PatchAdmissionStatus(ctx, s.client, wl, s.clock, func(wl *kueue.Workload) (bool, error) {
			updated := workload.UnsetQuotaReservationWithCondition(wl, reason, message, s.clock.Now())
		...
		s.recorder.Eventf(e.Obj, nil, corev1.EventTypeWarning, reason, reason, api.TruncateEventMessage(message))
	}
}
...
func isNoFitDueToCapacity(assignment *flavorassigner.Assignment) bool {
	...
	for _, ps := range assignment.PodSets {
		...
		for _, att := range ps.FlavorAssignmentAttempts {
			if att.Code == flavorassigner.CodeInsufficientQuota {
