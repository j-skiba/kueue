	...
	for iterator.hasNext() {
		...
		if e.assignment.RepresentativeMode() == flavorassigner.Preempt {
			...
			if preempted != 0 {
				...
				s.cache.ReserveResources(&e.Info, usage.Quota, victimKeys, e.clusterQueueSnapshot.Name, int64(priority.Priority(e.Obj)))
			} else if errors > 0 {
...
func (s *Scheduler) getInitialAssignments(log logr.Logger, wl *workload.Info, snap *schdcache.Snapshot) (flavorassigner.Assignment, []*preemption.Target) {
	...
	flvAssigner := flavorassigner.New(wl, cq, snap.ResourceFlavors, fairsharing.Enabled(s.fairSharing), preemption.NewOracle(s.preemptor, snap), replaceableWorkloadSlice, wlReservation, snap.AccountingReservations, snap.NominatedReservations)
