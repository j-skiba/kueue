...
func (c *Cache) recordCohortInfo(cohort *cohort, rootCohort *cohort) {
	...
	metrics.ReportCohortInfo(cohort.Name, newParent, newRoot, cohort.HasEffectiveQuota, cohort.EffectiveQuotaManager, c.customLabels.CohortGet(cohort.Name), c.roleTracker)
}
...
func (c *Cache) recordCQInfo(cq *clusterQueue, parentCohort kueue.CohortReference, rootCohort kueue.CohortReference) {
	...
	metrics.ReportClusterQueueInfo(cq.Name, parentCohort, rootCohort, cq.HasEffectiveQuota, cq.EffectiveQuotaManager, customLabels, c.roleTracker)
}
