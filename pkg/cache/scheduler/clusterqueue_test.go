...
func TestClusterQueueEffectiveQuotas(t *testing.T) {
	cq := utiltestingapi.MakeClusterQueue("cq").
		ResourceGroup(
			*utiltestingapi.MakeFlavorQuotas("x86").Resource(corev1.ResourceCPU, "10").Obj(),
		).
		EffectiveResourceGroup(
			*utiltestingapi.MakeFlavorQuotas("x86").Resource(corev1.ResourceCPU, "50").Obj(),
		).
		Obj()
	...
	cases := map[string]struct {
		...
		"DynamicQuota enabled uses effective quotas": {
			dynamicQuota: true,
			wantNominal:  50000,
		},
	}
