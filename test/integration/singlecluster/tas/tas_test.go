			...
			ginkgo.It("should not add unhealthy node when node is deleted but late pods exist", func() {
				...
				ginkgo.By("make one pod terminating on x3", func() {
					...
					for _, podCopy := range []*corev1.Pod{pod0, pod1} {
						if podCopy.Spec.NodeName == nodeName || (podCopy.Spec.NodeSelector != nil && podCopy.Spec.NodeSelector[corev1.LabelHostname] == nodeName) {
				...
				// Cleanup: finalizer removal on the pod to avoid leaking it
				ginkgo.By("cleanup pod finalizer", func() {
					...
					gomega.Eventually(func(g gomega.Gomega) {
						...
						_ = k8sClient.Delete(ctx, podToCleanup)
					}, util.Timeout, util.Interval).Should(gomega.Succeed())
				})
			})

			ginkgo.It("should not add unhealthy node when Ready condition is missing but late pods exist", func() {
				var wl *kueue.Workload
				nodeName := nodes[0].Name
				var terminatingPodName string
