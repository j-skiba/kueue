...
var _ = ginkgo.Describe("Preemption Loop Reproduction with Sleep and 0 Nominal Quota", ginkgo.Label("feature:fairsharing"), func() {
	...
	ginkgo.BeforeEach(func() {
		...
		features.SetFeatureGateDuringTest(ginkgo.GinkgoTB(), features.PreadmissionReservations, true)
		...
		go func() {
			for {
				...
				case <-autoEvictionCtx.Done():
					return
				default:
					for _, wlObj := range []*kueue.Workload{wlcq1, wlcq2Running, wlcq2New} {
						var wl kueue.Workload
						if err := k8sClient.Get(autoEvictionCtx, client.ObjectKeyFromObject(wlObj), &wl); err == nil {
