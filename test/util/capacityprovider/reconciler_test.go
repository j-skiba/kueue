...
func TestReconciler(t *testing.T) {
	cases := map[string]struct {
		...
		"unsupported parameters apiGroup": {
			enableFeatureGate: true,
			initObjects: []client.Object{
				utiltestingalpha.MakeCapacityProvider("cp").
					ControllerName(TestCapacityProviderControllerName).
					OrchestratedFlavors("f1").
					Parameters("custom.io", "ConfigMap", "cm").
					Obj(),
			},
			reqName: "cp",
			wantCondition: &metav1.Condition{
				...
				Status:  metav1.ConditionFalse,
				Reason:  kueuealpha.CapacityProviderReasonMisconfigured,
				Message: `unsupported parameters: expected APIGroup "k8s.io", got "custom.io"`,
			},
		},
		...
		"empty flavors in capacity YAML": {
			enableFeatureGate: true,
			initObjects: []client.Object{
				utiltestingalpha.MakeCapacityProvider("cp").
					ControllerName(TestCapacityProviderControllerName).
					...
					Obj(),
				MakeCapacityConfigMap("cm", metav1.NamespaceDefault).
					RawData(CapacityConfigMapKey, MustMarshalCapacityConfig(TestCapacityConfig{})).
					Obj(),
			},
			reqName: "cp",
			wantCondition: &metav1.Condition{
				...
				Status:  metav1.ConditionFalse,
				Reason:  kueuealpha.CapacityProviderReasonInvalidCapacity,
				Message: "must specify at least one flavor",
			},
		},
		"duplicate flavor in capacity YAML": {
			enableFeatureGate: true,
			initObjects: []client.Object{
				utiltestingalpha.MakeCapacityProvider("cp").
					ControllerName(TestCapacityProviderControllerName).
				...
				MakeCapacityConfigMap("cm", metav1.NamespaceDefault).
					RawData(CapacityConfigMapKey, MustMarshalCapacityConfig(TestCapacityConfig{
						Flavors: []TestCapacityFlavor{
							...
							{Name: "f1", Resources: corev1.ResourceList{corev1.ResourceCPU: resource.MustParse("2")}},
						},
					})).
					Obj(),
			},
			...
			wantCondition: &metav1.Condition{
				...
				Status:  metav1.ConditionFalse,
				Reason:  kueuealpha.CapacityProviderReasonInvalidCapacity,
				Message: `duplicate flavor name "f1"`,
			},
		},
		"empty flavor name in capacity YAML": {
			enableFeatureGate: true,
			initObjects: []client.Object{
				utiltestingalpha.MakeCapacityProvider("cp").
					ControllerName(TestCapacityProviderControllerName).
			...
			wantCondition: &metav1.Condition{
				...
				Status:  metav1.ConditionFalse,
				Reason:  kueuealpha.CapacityProviderReasonInvalidCapacity,
				Message: "flavor name cannot be empty",
			},
		},
		"flavor with no resources in capacity YAML": {
			enableFeatureGate: true,
			initObjects: []client.Object{
				utiltestingalpha.MakeCapacityProvider("cp").
					ControllerName(TestCapacityProviderControllerName).
			...
			wantCondition: &metav1.Condition{
				...
				Status:  metav1.ConditionFalse,
				Reason:  kueuealpha.CapacityProviderReasonInvalidCapacity,
				Message: `flavor "f1" must have between 1 and 64 resource entries`,
			},
		},
		...
		"filters un-orchestrated flavors and allows subset": {
			enableFeatureGate: true,
			initObjects: []client.Object{
				utiltestingalpha.MakeCapacityProvider("cp").
					ControllerName(TestCapacityProviderControllerName).
